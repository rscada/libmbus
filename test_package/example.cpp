...
int main(int argc, char **argv) {
  ...
  if (argc == 2) {
    ...
    device = argv[5];
             ...
             strcmp(argv[2], "-b") == 0 && strcmp(argv[4], "-r") == 0) {
    debug = 1;
    baudrate = atol(argv[3]);
    retries = atoi(argv[5]);
    device = argv[6];
  ...
  if (mbus_connect(handle) == -1) {
    fprintf(stderr,
            "Scan failed: Could not setup connection to M-bus gateway: %s\n",
            mbus_error_str());
    return 1;
  }

  if (mbus_context_set_option(handle, MBUS_OPTION_MAX_SEARCH_RETRY, retries) ==
      -1) {
  ...
  if (debug) printf("Scanning primary addresses:\n");
