...
main(int argc, char **argv)
{
    ...
    {
        fprintf(stderr, "usage: %s [-d] [-b BAUDRATE] mbus-address device AES128 (in 16 bytes) \n", argv[0]);
    ...
    // send the request
    //
    if (mbus_send_global_aes128_key(handle, address, text) == -1)
    {
        fprintf(stderr, "Send global AES128 key error: %s\n", mbus_error_str());
        return 1;
    }
